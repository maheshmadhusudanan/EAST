...
class Phase1_1ImageSegmentorHandler(PatternMatchingEventHandler):
    ...
    #
    # checks if the given rect has any other rectangle inside
    #
    def is_leaf(self,rect, bound_rects):
        
        for r1 in bound_rects:

            if CommonUtils.is_rectangle_inside(CommonUtils.get_coords(rect), CommonUtils.get_coords(r1)):              
                return False                
    ...
    def process(self, event):
        ...
        for c in contours:       
            ...
            if ((w > 20 and h > 10) or (w > 10 and h > 20 and h < 1200)) :
                r = {'x':x-MARGIN,'y':y-MARGIN,'w':w+MARGIN,'h':h+MARGIN}                
                n = SimpleNamespace(**r)
        ...
        with open(res_file, 'w') as f:
            f.write('{},{},{},{},{},{}\r\n'.format("file", "x1", "y1", "x2", "y2","isleaf"))
            for r in bounding_rects:
                ...
                print("******************************************* isl = "+str(isl))
                f.write('{},{},{},{},{},{}\r\n'.format(process_pipeline_file.task_output_file_name,rc.x1, rc.y1, rc.x2, rc.y2, str(isl)))
